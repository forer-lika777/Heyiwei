...
void App::run()
{
 //程序开始运行的地方
  ...
  // 判断输入的序号是否在操作选项列表内
		...
		switch (operation) {
		case 1:
			listAllStudents(manager); // 列出所有学生列表
			break;
		case 2:
			addStudent(manager); // 添加学生
			break;
		}
	}
}
...
void App::listAllStudents(WaterManager& manager)
{
   ...
   // 判断输入的 s[学号] 格式
			if (input.length() >= 2 && (input[0] == 's' || input[0] == 'S')) {
				...
				operateOnStudent(manager, numStr); // 打开修改学生信息页面
				break;
			}
...
void App::operateOnStudent(WaterManager& manager, const std::string id) {
	...
	while (1) {
		...
		switch (operation) {
		case 1:
			listAllRecords(manager, id); // 列出所有水费记录列表
			break;
		case 2:
			// 未完成
			break;
		case 3:
			addWaterRecord(manager, id); // 添加水费记录
			break;
		case 4:
